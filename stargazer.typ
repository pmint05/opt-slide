...
#let slide(
  ...
  if navigation != auto {
    self.store.navigation = navigation
  }
  if header != auto {
  ...
  if progress-bar != auto {
    self.store.progress-bar = progress-bar
  }
...
#let title-slide(..args) = touying-slide-wrapper(self => {
  ...
  info.instructors = {
    let instructors = if "instructors" in info {
      info.instructors
    } else {
      info.instructor
    }
    if type(instructors) == array {
      instructors
    } else {
      (instructors,)
    }
  ...
  let body = {
    ...
    align(center, [
      #grid(
        columns: (auto, auto) * calc.min(info.authors.len(), 3),
        align: (left, left),
        column-gutter: 1.5em,
        row-gutter: 1em,
        [Học viên thực hiện:],
